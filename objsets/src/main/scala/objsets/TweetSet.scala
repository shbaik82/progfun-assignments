...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedHelper(tweet: Tweet): Tweet =
    if (tweet.retweets > elem.retweets) right.mostRetweetedHelper(left.mostRetweetedHelper(tweet))
    else right.mostRetweetedHelper(left.mostRetweetedHelper(elem))
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = tweetHelper(google)
  lazy val appleTweets: TweetSet = tweetHelper(apple)
  
  def tweetHelper(keys: List[String]): TweetSet = 
    if (keys.isEmpty) new Empty
    else TweetReader.allTweets.filter(tw => tw.text.contains(keys.head)).union(tweetHelper(keys.tail))
